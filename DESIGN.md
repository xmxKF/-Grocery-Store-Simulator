...
阴影 pass **249** 个深度 draw（**D-T7 闸门场景实测**，与 §5.8 的 727 同一场景同一次运行；口径 = 全场 `castShadow===true` 且自身与全部祖先 `visible!==false` 的 Mesh。C-终审的 36 箱场景为 237，D 期把 12 只散落地面箱纳入闸门后 +12）：
...
| 纸箱箱体 | 48 | 48 只箱 × 1（仓库 24 + 卸货 12 + 散落地面 12 = `BOX_HARD_CAP`；`0.45³`，类目色标签面片不投影） |
...
总提交 **976** = 727 主渲染 + 249 阴影深度（**D-T7 实测**，`perf.renderCalls` 实读 `renderer.info.render.calls`，天花板 1050、余量 74；见 §5.8 的两个口径表）。lowfx 实测 **727**：`shadowMap.enabled=false` 一个布尔，阴影 pass 归零。

**D 期闸门场景已含 12 只散落箱**（§5.8 的 727/760）；散落箱箱体同样 `castShadow=true`（`world.js` 的 `spawnBox`），已计入上表的 48 与本节合计。
...
| 自测现场实测 | **727** | **D-T7 实测回写**（`perf.mainPassDrawables` 现场普查，正常态与 lowfx 同值）|

**总闸门 550→760**，余量 **33**（D-T7 实测 727；D 期把 12 只散落地面箱纳入闸门场景后，余量从 57 收窄到 33）。

- **【D 期终审订正：D-T7 的那条订正本身是错的，已作废】** D-T7 写过「散件商品 mesh（非实例池）共 146，表里顾客手持只列 120，差额 26 疑为传送带商品，故本表构成对不上」。终审在**同一取样点**（`perf.mainPassDrawables` 之前）独立重做普查，**证伪了这条订正**：`146 − 122 = 24` 恰好等于本表「实例池」那一行——**D-T7 的普查虽标了「非实例池」，实际把 24 个 InstancedMesh 一并数了进去**。

  **终审普查（口径：几何 === `G.world.itemGeoFor(pid)` 返回的共享商品基础几何、且自身与全部祖先 `visible !== false`；InstancedMesh 单独归类）：**

  | 项 | 表列 | 实测 | |
  |---|---:|---:|---|
  | 价签（按 `slot.tagMesh` 点名） | 120 | **120** | ✓ |
  | 存储位标记（按 `storageSlot.marker` 点名） | 24 | **24** | ✓ |
  | 收银台（按 `register.mesh` / `.beltMesh` 点名） | 6 | **6** | ✓ |
  | 箱 mesh（仓库 48 + 卸货 24 + 散落 24，按 `box.body` / `.label` 点名） | 96 | **96** | ✓ |
  | 壳体（按 `userData.shell` 点名） | 21 | **21** | ✓ |
  | 实例池（`isInstancedMesh`，全部 24 个几何都是共享商品几何） | 24 | **24** | ✓ |
  | 顾客手持（散件商品 mesh，挂在 20 个顾客组下） | 120 | **120** | ✓ |
  | 收银传送带上的商品 mesh | —（列在「未纳入闸门场景」） | **0** | ✓ 「未纳入」成立 |
  | 上架飞行动画的临时 mesh | **本表无此行** | **2** | ← 表缺的 **+2** |
  | 余下未点名（货架框 104 + 设施 4 + 卷帘门 0 + 顾客 180 + 收银员 27） | 315 | **314** | ← **−1** |
  | **合计** | **726** | **727** | |

  **结论：`727 = 726 + 2 − 1`。** 表列「顾客手持 120」实测一个不差；「传送带商品未纳入闸门场景」实测为真（取样点 0）。真正缺的一行是**上架飞行动画的 2 只临时 mesh**（`G.world._flights`，取样时恰有 2 只在飞）。**唯一仍未逐行核实的是那个 −1**，已定位在「货架框 / 设施 / 卷帘门 / 顾客 / 收银员」这一组共 315 的行项之内（这五行没有可逐个点名的对象表，只能靠重数几何）；`CylinderGeometry` 在该组只剩 1 个而「设施 4」含桶身+桶盖两件，是**待验的**嫌疑点，未经证实不得写进表里。
  **下次改本表前请先重做一次上表口径的普查**，不要在现有行项上做加减。闸门本身不受影响：`countDrawables` 数的是现场实况，不读这张表。
...
- 阴影深度 pass **249**、一帧总提交 **976**（**D-T7 实测**，48 箱满配闸门场景；C-终审的 36 箱场景是 237/939，差 12 箱 = 深度 12 + 主 pass 24）。
...
**未纳入闸门场景的负载**（下次加装饰前须先补进来）：传送带上的商品 mesh（**D 期终审实测：取样点确为 0**，本行成立）、玩家举起的纸箱。余量 **33** 比看上去薄得多 —— 散落地面箱已于 D 期纳入闸门场景（12 只 × 2 = 24），下一次加装饰前必须先重估这 33 格。
