...
- **【D-T7 订正：本表的总数对得上，构成对不上】** 逐几何普查现场 727 个 drawable，与「理论满配合计 726」只差 1，但这个 1 是巧合而非表的精度。实测构成中，散件商品 mesh（非实例池）共 **146** 个，而表里「顾客手持（上限 6/人）」只列 120；差额 26 疑为收银传送带上的商品 mesh —— 本节下文把它列为「未纳入闸门场景的负载」，实际上它一直在闸门场景里。**下次改本表前必须先逐行重做普查**，不要在现有行项上做加减。
...
**未纳入闸门场景的负载**（下次加装饰前须先补进来）：传送带上的商品 mesh、玩家举起的纸箱。余量 **33** 比看上去薄得多 —— 散落地面箱已于 D 期纳入闸门场景（12 只 × 2 = 24），下一次加装饰前必须先重估这 33 格。
