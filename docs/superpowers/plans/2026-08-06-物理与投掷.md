...
| 文件 | D 期改动 |
|---|---|
| `docs/GDD.md` | T0：新增「§15 物理与投掷」节；**§12 操作表补右键（三张活镜像之一，见下）**；§13 UI 清单补 `#charge` |
...
| `js/ui.js` | T3：`setCharge(v)` + `#charge` 构建；**`HELP_ROWS` 补右键（操作表镜像之二）** |
| `css/style.css` | T3：`#charge` 样式 |
| `js/main.js` | T1：`boot` / `tick` 接入 + 断言 1-5、12；T2-T6 各自断言；T7：断言收口 + 闸门扩场景 |
| `README.md` | T3：**§操作表补右键（操作表镜像之三）** |

> **【D-T7 登记】操作表有三张活镜像，改键位必须三处同改**：`docs/GDD.md §12` ↔ `js/ui.js` 的 `HELP_ROWS`（玩家在主菜单实际看到的那张）↔ `README.md §操作表`。**没有任何断言能守**（`file://` 下不能 XHR 读 `.md`）。本表在 D-T0 只登记了 GDD 一处，正是 T3 漏掉另两处的根因；同款清单已写进 `docs/CONTRACTS.md` 的固定清单区。
